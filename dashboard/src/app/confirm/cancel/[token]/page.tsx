...
export default function CancelPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`${API}/confirm/cancel/${token}`).then(async (r) => ({ res: r, body: await r.json() })),
      minWait,
    ])
      .then(([{ res, body }]) => {
  ...
  return (
    <MerchantBrandedLayout>
      <AnimatePresence mode="wait">
        {state.kind === "loading" && <LoadingState key="loading" label={t("cod_cancel_loading")} />}
        {state.kind === "success" && (
          <SuccessState
            key="success"
            order={state.order}
            title={t("cod_cancel_success_title")}
            intro={fillTemplate(t("cod_cancel_success_intro"), {
  ...
  return (
    ...
    >
      <div className="relative h-16 w-16">
        ...
        <span
          aria-hidden
          className="absolute inset-0 animate-spin rounded-full border-2 border-transparent border-t-[var(--color-fg-muted)]"
        />
      </div>
...
function SuccessState({
  ...
  return (
    ...
    >
      ...
      <p className="mt-2 max-w-[28ch] text-[15px] text-[var(--color-fg-muted)]">{intro}</p>
      <p className="mt-1 max-w-[32ch] text-[14px] text-[var(--color-fg-faint)]">
        Yeni sipariş için mağazayı ziyaret edebilirsin.
      </p>
...
function CancelMark() {
  return (
    <motion.div
      ...
      className="flex h-20 w-20 items-center justify-center rounded-full border border-[var(--color-fg-muted)]/20 bg-[var(--color-surface)]"
    >
      <XCircle
        size={42}
        strokeWidth={1.6}
